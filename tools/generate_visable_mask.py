...
if __name__=="__main__":
    ...
    # for i, batch in enumerate(data_loader):
        ...
        # info = data_infos[data_id]
        info =data_infos[i]
        # if os.path.exists(info['occv2_path'].replace('openocc_v2','openocc_v2_mask1015')):
        #     # print(i,"finisned")
        #     continue
        # else:
        #     print(i,"unfinished")
        ...
        for coord_index in coord_indexs:                  
            ...
            if gt_semantics[coord_index[0],coord_index[1],coord_index[2]]<16:
                ...
                if gt_semantics[coord_index[0],coord_index[1],coord_index[2]]<10:
                    dist_add_values=np.linspace(0, min(5,0.5*dist), 7)#包含首尾
                else:
                    dist_add_values=np.linspace(0, min(5,0.5*dist), 7)
        ...
        #save
        occ_gt['vismask']=vismask
        save_path=info['occv2_path'].replace('openocc_v2','openocc_v2_obs11_nextrend1025')
        if save_path[-4:]!=".npz":
            save_path=save_path+"/labels.npz"
        if not os.path.exists(os.path.dirname(save_path)):
            os.makedirs(os.path.dirname(save_path))

...
class PlanUAutoRegTransformer(BaseModule):
    ...
    def forward(self, tokens, pose_tokens):
        ...
        # tokens: bs, f, c, h, w #b,15,128,50,50
        # pose_tokens, bs, f, c
        ...
        for temporal_attn, encoder, down, pose_attn_en, pose_en in zip(self.temporal_attentions_en, self.encoders, self.downsamples, self.pose_attn_en, self.pose_en):
            ...
            for pose_temporal_attn, pose_temporal_norm, spatial_attn, spatial_norm, ffn, ffn_norm in pose_attn_en:
                pose_queries = pose_queries + pose_temporal_attn(pose_queries, pose_tokens, pose_tokens, need_weights=False, attn_mask=self.attn_mask)[0]#[b,15,128]
                ...
                pose_queries = pose_queries + spatial_attn(pose_queries, queries, queries, need_weights=False, attn_mask=None)[0]#[T,B,128]
            ...
            pose_queries = pose_en(pose_queries)#mlp
            ...
            #queries = rearrange(queries, 'b f h w c -> (b h w) f c')
            for cross_attn, cross_norm, ffn, ffn_norm in temporal_attn:#6
        ...
        queries = self.mid(queries)#unet
        tokens = self.mid(tokens)
        
        pose_queries = self.pose_mid(pose_queries)#mlp
        ...
        queries = self.conv_out(queries)#[B,15,128->512,50,50]
        pose_queries = self.pose_out(pose_queries)#[B,15,128]

...
class YOLOXHeadCustom(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        if self.vis_idx%200==10 and flatten_bboxes.device==torch.device('cuda:0'):
    ...
    def _get_target_single(self, cls_preds, objectness, priors, decoded_bboxes,
        ...
        gt_bboxes = gt_bboxes.to(decoded_bboxes)#to(decoded_bboxes.dtype)
    ...
    def _get_l1_target(self, l1_target, gt_bboxes, priors, eps=1e-8):
        ...
        gt_cxcywh = bbox_xyxy_to_cxcywh(gt_bboxes)#输入已经处理成xywh格式
        # gt_cxcywh=gt_bboxes

...
class TransVQVAE(BaseModule):
    ...
    def forward_train_with_plan(self, x, metas):
        ...
        assert F == self.num_frames + self.offset
        output_dict = {}
        z, shape = self.vae.forward_encoder(x)#[B,16,200,200,Z]
        z = self.vae.vqvae.quant_conv(z)#[16,16*8,50,50]
        z_q, loss, (perplexity, min_encodings, min_encoding_indices) = self.vae.vqvae.forward_quantizer(z, is_voxel=False)
        min_encoding_indices = rearrange(min_encoding_indices, '(b f) h w -> b f h w', b=bs)
        output_dict['ce_labels'] = min_encoding_indices[:, self.offset:].detach().flatten(0,1)#1~15 后15帧
        z_q = rearrange(z_q, '(b f) c h w -> b f c h w', b=bs)
        hidden = None
        if self.give_hiddens:
            hidden = z_q[:, :self.offset]
            
        #rel_poses b, F-self.offset=self.num_frames=15,128  
        rel_poses, output_metas = self._get_pose_feature(metas, F-self.offset)
        #z_q去掉了最后一帧 [B,self.num_frames,128,50,50]
        z_q_predict, rel_poses = self.transformer(z_q[:, :self.num_frames], pose_tokens=rel_poses)#rel_poses:[B,15,128]
        
        pose_decoded = self.pose_decoder(rel_poses)#[B,15,128]
        output_dict['pose_decoded'] = pose_decoded#[B,15,3(num_nodes),2]
        output_dict['output_metas'] = output_metas
        
        
        z_q_predict = z_q_predict.flatten(0, 1)#[B,15,512,50,50]
        output_dict['ce_inputs'] = z_q_predict#[T(15)*b,512,50,50]
        # z: bs*f, c, h, w 
    ...
    def _get_pose_feature(self, metas=None, F=None):
        ...
        if hasattr(self, 'pose_encoder'):
            ...
            if F>self.num_frames:
                ...
                gt_mode = gt_mode[:, :-self.offset, :] #前15帧[1,16-1,3] 0,0,1
                rel_poses = rel_poses[:, :-self.offset, :]#[1,16-1,2]

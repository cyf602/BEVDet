...
class BEVStereo4DOCC(BEVStereo4D):
    ...
    def loss_single(self,voxel_semantics,preds_occ,voxel_flow=None,preds_flow=None,
        ...
        if preds_flow is not None:
            ...
            if self.use_future_loss:#自监督
                ...
                loss_['loss_flow_fut']=self.flow_futureloss(next_voxel_semantics,preds,final_mask,transform_martix,dstamp=dstamp)
            else:        
    ...
    #     """
    #     occ_pred,flow_pred=outs['occ_results'],outs['flow_results']
    #     # occ_pred=occ_pred.view(-1,self.occ_zdim,self.bev_w,self.bev_h,self.num_classes).permute(0,3,2,1,4)
    #     B,H,W,Z,nc=occ_pred.shape#1,200,200,16,~16
    #     flow_pred=flow_pred.view(B,H,W,Z,2)
    #     final_mask=final_mask.view(B,H,W,Z)
    #     locpr_c2t2=self.local_voxel_coors.to(flow_pred.device)#当前坐标系下下一帧体素坐标
    #     locgt_c1t2=self.local_voxel_coors.to(flow_pred.device)#下一帧坐标系下体素坐标
    #     # loc_t=self.local_voxel_coors.repeat(B,1,1,1,1).to(flow_pred.device)
    #     dstamp=dstamp.view(B,1,1,1)
    #     #当前帧坐标系下 下一帧体素gt坐标
    #     locpr_c1t1=torch.stack((locpr_c1t2[...,0]-flow_pred[...,0]*dstamp,locpr_c1t2[...,1]+flow_pred[...,1]*dstamp,locpr_c1t1[...,2].repeat(B,1,1,1),torch.ones_like(locpr_c1t1[...,0]).repeat(B,1,1,1)),dim=-1)#B, h w z,4
    #     # loc_t2=torch.matmul(transform_martix[None,None,None,...],locnext.unsqueeze(-1)).squeeze(-1)#移动后在下一帧坐标系下的坐标 [1,1,1,1,4,4]*[B,H,W,Z,4,1]
    #     locpr_c2t2=torch.einsum('bxy,bwhzy->bwhzx',transform_martix,locpr_c1t2)#下一帧在当前帧坐标系下坐标
    #     locgt_c2t2=torch.einsum('bxy,bwhzy->bwhzx',transform_martix,locgt_c2t2)#下一帧在当前帧坐标系下坐标
    #     #locpr_c2t2 只用于算范围内的mask，移动后在下一帧坐标系下的坐标索引  long()之后没梯度 故后面用grid sample
    #     loc_t2_idx=((locpr_c2t2[...,:3]-torch.tensor(self.pc_range[:3]).to(flow_pred.device))/self.res).long()# loc_t2 to idx [B,H W Z,3]#最后维度每个元素都是表示
    #     mask2=(0<=loc_t2_idx[...,0])&(loc_t2_idx[...,0]<H) \
    #         &(0<=loc_t2_idx[...,1])&(loc_t2_idx[...,0]<W)\
    #         &(0<=loc_t2_idx[...,2])&(loc_t2_idx[...,2]<Z) #范围限制
    #     #每个occ_pred都通过对应位置的loc_t2_idx转换成occ_infer_next中对应位置的预测
    #     # h_idx,w_idx,z_idx=loc_t2_idx[...,0].clamp(0,H-1),loc_t2_idx[...,1].clamp(0,W-1),loc_t2_idx[...,0].clamp(0,Z-1)
    #     # b_idx=torch.arange(B)[:,None,None,None]
    ...
    #     occ_gt_next2cur=F.grid_sample(occ_next.unsqueeze(1).to(torch.float64),locgt_c2t2[...,:3]/torch.tensor(self.occupancy_size).to(occ_next.device),mode='nearest')
    #     occ_gt_next2cur=occ_gt_next2cur.squeeze(1)
    #     # occ_gt_next2cur=torch.ones(B,H,W,Z).to(occ_pred.device)*(nc-1)#全置free
    #     # occ_gt_next2cur=occ_next[b_idx,h_idx,w_idx,z_idx]#下一帧gt在当前帧对应位置下
    #     final_mask*=mask2
    #     # occ_infer_next=torch.zeros(B,H,W,Z,nc,dtype=torch.float64).to(occ_pred.device)
    #     # occ_infer_next[b_idx,h_idx,w_idx,z_idx]=occ_pred
    #     # occ_infer_next=torch.zeros(torch.max(h_idx),torch.max(w_idx),torch.max(z_idx))
    #     # occ_infer_next=occ_pred[b_idx,h_idx,w_idx,z_idx,:]#[B,200,200,16,nc]
    #     # final_mask_next=final_mask[b_idx,h_idx,w_idx,z_idx]
    #     # mask2=torch.logical_and(mask2,final_mask_next)
    #     num_valid=torch.sum(final_mask)
    #     print('num_valid:',num_valid)
    #     if num_valid==0:
    #         return 0*self.future_loss(occ_pred.reshape(-1,nc),occ_gt_next2cur.long().reshape(-1),avg_factor=1)#mask2
    #     else:
    #         return self.future_loss(occ_gt_next2cur.long()[final_mask],occ_pred[final_mask],avg_factor=torch.sum(final_mask))
    ...
    def forward_train(self,
        ...
        loss_occ = self.loss_single(voxel_semantics, occ_pred,voxel_flow,flow_pred,mask_camera=vismask,next_voxel_semantics=next_voxel_semantics,dstamp=dstamp,transform_martix=ego2next_mat)       
